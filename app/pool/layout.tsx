...
export default function PoolLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (session?.user) {
        setDisplayName(session.user.user_metadata?.display_name || session.user.email?.split('@')[0] || 'Player')
      }
    }
    load()
  }, [])
      ...
      <nav className="sticky top-0 z-50 border-b" style={{background: 'var(--pitch)', borderBottomColor: 'rgba(245,240,232,0.1)'}}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={handleLogout} className="text-xs font-display tracking-widest" style={{color: 'var(--chalk-dim)'}}>
              LOG OUT
            </button>
          </div>
        </div>
